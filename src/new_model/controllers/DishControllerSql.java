...
public class DishControllerSql extends DbController implements DishController {
    ...
    public String getReadableInfo() {
        String answ = "";
        SelectResult res = dishTableController.select(new String[] {"name"});
        for (KeyVals name:
             res) {
    ...
    public Dish getDish(String dishName) {
        if (!dishTableController.contains("name", dishName))
            return null;
        KeyVals resDish = dishTableController.selectOne(ALL, "name='" + dishName + "'");
        if (resDish == null)
            return null;
    ...
    public Recipe getReciepe(int dishId, int people) {
        ...
        if (!dishTableController.contains("dish_id", idStr))
            return null;
        ...
        return new Recipe(foodController, ingredients, weights, people);
    }

    @Override
    public ArrayList<String> getDishesWithType(int type) {
        ArrayList<String> answ = new ArrayList<>();
        SelectResult res = dishTableController.select(ALL, "dish_type_id='" + type + "'");
        for (KeyVals cur:
             res) {
    ...
    public boolean contains(String name) {
        return dishTableController.contains("name", name);
    }
    ...
    public void addDish(String dishName, ArrayList<Integer> ingredients, ArrayList<Integer> weights, int peopleAmount, int dishType) {
        ...
        KeyVals dishIdKv = dishTableController.selectOne(ALL, "name='"+dishName+"'");
        String dishId = dishIdKv.get("dish_id");
        for (int i = 0; i < ingredients.size(); i++){
            String ingr = String.valueOf(ingredients.get(i));
            String amount = String.valueOf(ingredients.get(i));
    ...
    public void addDishName(String name, int type, int people){
        ...
        dishTableController.insert(values);
    }
}
